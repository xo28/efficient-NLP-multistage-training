...
from dataset import tokenize_fn_amz
...
dataset = None
if actual_task in ['amazon_polarity', 'amazon', 'amz']:
    actual_task = 'amazon_polarity'
    dataset = load_dataset("amazon_polarity")
    dataset_tokenized = dataset.map(tokenize_fn_amz, batched=True)
    dataset_tokenized.rename_column("label", "labels")
    dataset_tokenized.set_format('torch')
    dataset_tokenized = dataset_tokenized.remove_columns(['content'])
    dataset_tokenized = dataset_tokenized.remove_columns(['title'])
...
if actual_task == 'amazon_polarity':
    trn_set = trn_set.shuffle().select(range(360000)) # We're limiting the dataset size to speed up the training during the demo
    val_set = val_set.shuffle().select(range(40000))
